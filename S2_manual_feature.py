...
def cat_feature(df):
    ...
    if lastk is None:
        num_agg_df = df.groupby("customer_ID", sort=False)[one_hot_features].agg(
            ["mean", "std", "sum", "last"]
        )
    else:
        num_agg_df = df.groupby("customer_ID", sort=False)[one_hot_features].agg(
    ...
    if lastk is None:
        cat_agg_df = df.groupby("customer_ID", sort=False)[cat_features].agg(
            ["last", "nunique"]
        )
    else:
        cat_agg_df = df.groupby("customer_ID", sort=False)[cat_features].agg(
...
def num_feature(df):
    if num_features[0][:5] == "rank_":
        ...
        if lastk is None:
            num_agg_df = df.groupby("customer_ID", sort=False)[num_features].agg(
...
def diff_feature(df):
    ...
    if lastk is None:
        num_agg_df = df.groupby("customer_ID", sort=False)[diff_num_features].agg(
            ["mean", "std", "min", "max", "sum", "last"]
        )
    else:
        num_agg_df = df.groupby("customer_ID", sort=False)[diff_num_features].agg(
...
for li, lastk in enumerate([None, 3, 6]):
    for prefix in transform[li]:
        df = (
            pd.read_feather(f"./input/train.feather")
            .append(pd.read_feather(f"./input/test.feather"))
        ...
        if prefix == "rank_":
            cids = df["customer_ID"].values
            df = (
                df.groupby("customer_ID")[num_features]
                .rank(pct=True)
                .add_prefix("rank_")
            )
            df.insert(0, "customer_ID", cids)
        ...
        if prefix in ["", "last3_"]:
            cat_feature_df = pd.concat(
                pool.map(cat_feature, tqdm(dfs, desc="cat_feature"))
            ).reset_index(drop=True)

            cat_feature_df.to_feather(f"./input/{prefix}cat_feature.feather")

        if prefix in ["", "last3_", "last6_", "rank_", "ym_rank_"]:
            num_feature_df = pd.concat(
                pool.map(num_feature, tqdm(dfs, desc="num_feature"))
            ).reset_index(drop=True)
            num_feature_df.to_feather(f"./input/{prefix}num_feature.feather")

        if prefix in ["", "last3_"]:
            diff_feature_df = pd.concat(
                pool.map(diff_feature, tqdm(dfs, desc="diff_feature"))
            ).reset_index(drop=True)
