...
class Amodel(nn.Module):
    def __init__(
        ...
        self.input_series_block = nn.Sequential(
            nn.Linear(series_dim, hidden_dim), nn.LayerNorm(hidden_dim)
        )
        ...
        self.output_block = nn.Sequential(
            nn.Linear(
                3 * hidden_dim if use_series_oof else 2 * hidden_dim, 1 * hidden_dim
            ),
            nn.LeakyReLU(),
    ...
    def batch_gru(self, series, mask):
        ...
        pack = nn.utils.rnn.pack_padded_sequence(
            series, node_num, batch_first=True, enforce_sorted=False
        )
